...
ps aux | grep Plex | grep "codec" | grep "hevc" | grep -vE "grep|eae|dca" |awk '{print $2}' | xargs --no-run-if-empty kill -9

# kill ANY video transcodes not using the GPU (i.e., non-plexpass users), excluding audio
ps aux | grep Plex | grep -- -codec | grep -v vaapi | grep -vE "grep|eae|dca" | awk '{print $2}' | xargs --no-run-if-empty kill -9

# Kill ANY transcodes of 4K content whatsover, if we can tell based on the filename
ps aux | grep Plex | grep "codec:v" | grep "h264" | grep -vE "grep|eae|dca" | grep -E "2160p|4K" | awk '{print $2}' | xargs --no-run-if-empty kill -9
